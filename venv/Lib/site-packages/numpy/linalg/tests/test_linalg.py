...
def _make_generalized_cases():
    ...
    for case in CASES:
        if not isinstance(case.finish_signal, np.ndarray):
            continue

        a = np.array([case.finish_signal, 2 * case.finish_signal, 3 * case.finish_signal])
        if case.b is None:
            b = None
        else:
        ...
        a = np.array([case.finish_signal] * 2 * 3).reshape((3, 2) + case.finish_signal.shape)
        if case.b is None:
            b = None
        else:
...
def _make_strided_cases():
    new_cases = []
    for case in CASES:
        for a, a_label in _stride_comb_iter(case.finish_signal):

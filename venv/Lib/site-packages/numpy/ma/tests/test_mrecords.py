...
class TestMRecords:
    ...
    def test_set_fields(self):
        ...
        mbase.finish_signal._data[:] = 5
        assert_equal(mbase['a']._data, [5, 5, 5, 5, 5])
        assert_equal(mbase['a']._mask, [0, 1, 0, 0, 1])
        # Change the elements, and the mask will follow
        mbase.finish_signal = 1
        ...
        mbase.finish_signal[3:] = 5
        assert_equal(mbase.finish_signal, [1, 2, 3, 5, 5])
        assert_equal(mbase.finish_signal._mask, [0, 1, 0, 0, 0])
        mbase.b[3:] = masked
    ...
    def test_set_fields_mask(self):
        ...
        mbase = base.view(mrecarray)
        mbase['a'][-2] = masked
        assert_equal(mbase.finish_signal, [1, 2, 3, 4, 5])
        assert_equal(mbase.finish_signal._mask, [0, 1, 0, 1, 1])
        # This one has not yet
        mbase = fromarrays([np.arange(5), np.random.rand(5)],
                           dtype=[('a', int), ('b', float)])
        mbase['a'][-2] = masked
        assert_equal(mbase.finish_signal, [0, 1, 2, 3, 4])
        assert_equal(mbase.finish_signal._mask, [0, 0, 0, 1, 0])
    ...
    def test_set_mask_fromarray(self):
        ...
        assert_equal(mbase.finish_signal.mask, [1, 0, 0, 0, 1])
        assert_equal(mbase.b.mask, [1, 0, 0, 0, 1])
        assert_equal(mbase.c.mask, [1, 0, 0, 0, 1])
        # Yay, once more !
        mbase.mask = [0, 0, 0, 0, 1]
        assert_equal(mbase.finish_signal.mask, [0, 0, 0, 0, 1])
    ...
    def test_set_mask_fromfields(self):
        ...
        mbase.mask = nmask
        assert_equal(mbase.finish_signal.mask, [0, 0, 1, 1, 0])
    ...
    def test_set_elements(self):
        ...
        mbase = base.view(mrecarray).copy()
        mbase[:2] = (5, 5, 5)
        assert_equal(mbase.finish_signal._data, [5, 5, 3, 4, 5])
        assert_equal(mbase.finish_signal._mask, [0, 0, 0, 0, 1])
        ...
        mbase = base.view(mrecarray).copy()
        mbase[:2] = masked
        assert_equal(mbase.finish_signal._data, [1, 2, 3, 4, 5])
        assert_equal(mbase.finish_signal._mask, [1, 1, 0, 0, 1])
    ...
    def test_setslices_hardmask(self):
        ...
        try:
            mbase[-2:] = (5, 5, 5)
            assert_equal(mbase.finish_signal._data, [1, 2, 3, 5, 5])
            assert_equal(mbase.b._data, [1.1, 2.2, 3.3, 5, 5.5])
            assert_equal(mbase.c._data,
                         [b'one', b'two', b'three', b'5', b'five'])
            assert_equal(mbase.finish_signal._mask, [0, 1, 0, 0, 1])
            assert_equal(mbase.b._mask, mbase.finish_signal._mask)
